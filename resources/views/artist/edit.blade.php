@extends('layouts.app')

@section('content')
    <div class="container">
        <div class="row">
            @include('admin.sidebar')

            <div class="col-md-9">
                <div class="card">
                    <div class="card-header">Artiste: {{ $artist->firstname }} {{ $artist->lastname }}</div>
                    <div class="card-body">
                        <a href="{{ url('/artist') }}" title="Retour">
                            <button class="btn btn-info btn-sm">
                                <i class="fa fa-arrow-left" aria-hidden="true"></i>
                                Retour
                            </button>
                        </a>
                        <br/>
                        <br/>

                        @if ($errors->any())
                            <ul class="alert alert-danger">
                                @foreach ($errors->all() as $error)
                                    <li>{{ $error }}</li>
                                @endforeach
                            </ul>
                        @endif

                        <form method="POST" action="{{ url('/artist/' . $artist->id) }}" accept-charset="UTF-8"
                              class="form-horizontal" enctype="multipart/form-data">
                            {{ method_field('PATCH') }}
                            {{ csrf_field() }}

                            @include ('artist.form', ['formMode' => 'edit'])

                        </form>


                        <hr>
                        <strong>Types</strong>
                        <ul>
                            @foreach($artist->artisteTypes()->get() as $artistType)
                                <li>{{ title_case($artistType->type()->first()->type) }}</li>
                            @endforeach
                        </ul>
                        <a href="{{ route('artistType.editForArtist', $artist->id) }}" class="btn btn-success">Éditer types</a>

                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
